...
float CScore::scoreRead(string read, int order, map<string, int> kmerMap, vector<float> model) {
	...
	while(first) {
		try {
	        ...
	        //        cout << "tmpKmer " << order << ": " << tmpKmer << "Mapval: " << mappedIndex  << " Score: " << model.at(mappedIndex) << "Partial score: " << score << "\n";
			first = false;
	...
	//cout << "tmpKmer " << order << ": " << tmpKmer << "Partial score: " << score << "\n";
	...
	//	cout << "tmpKmer " << j << ": " << tmpKmer << " ";
        ...
        //cout << "Mapval: " << mappedIndex << " Score: " << model.at(mappedIndex) << " ";
			score += model.at(mappedIndex);
	//cout << "New Partial Score: " << score << endl;
	...
	if (listFile.is_open()) {
		...
		if (scoreResults.is_open()) {
			scoreResults << "Best score\tBest model\n";
                        //std::cout << this->scores.size() << endl;
